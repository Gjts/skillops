...
function fixtureArtifact(id, version, contents) {
  return {
    artifact: normalizeArtifactDefinition({
      ...
      version,
      source: 'github',
      sourceRef: `github:skillops/deterministic-fixture#${id}`,
...
export async function evaluationRun(args, dependencies = {}) {
  ...
  const inputs = deterministic
    ...
    : {
        baseline: await (dependencies.artifacts || createArtifactResolver(dependencies)).resolve(baselineRef),
        candidate: await (dependencies.artifacts || createArtifactResolver(dependencies)).resolve(candidateRef),
